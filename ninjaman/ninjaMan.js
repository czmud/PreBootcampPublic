...
function populateGridWithSushiSpaces(worldFx) {
	...
	for (let z = 0; z < totalSushiSpaces; z++) {
		...
		if (nextSushiSpace === 0) {
			if (branchCount < numberOfBranches) {
				for (let w = 0; w < mapSpaceOrder.length; w++) {
					//time to back up 'n branch
	...
	for (let y = 0; y < worldHeight; y++) {
		//count total number of sushi in game
		for (let x = 0; x < worldLength; x++) {
...
function returnAGoodSpace(topFx, leftFx, nextSushiSpace) {
	...
	for (let z = 1; z <= 4; z++) {
		if (!isAnEdge(topFx, leftFx, z) && isSquareAWall(topFx, leftFx, z)) {
			goodSpaceExists = true;
		}
	}
	if (goodSpaceExists) {
		nextSushiSpace = Math.ceil(Math.random() * 4);
		if (isAnEdge(topFx, leftFx, nextSushiSpace) || !isSquareAWall(topFx, leftFx, nextSushiSpace)) {
...
function sushiGenProbTrue(topFx, leftFx, nextSushiSpace) {
	//returns true for sushi space generation based on probability rules
...
function generateGhosts() {
	...
	for (let x = 0; x < 5; x++) {
		...
		while (world[ghostTop][ghostLeft] === 0) {
			...
			if (ghostLeft < 5 && ghostTop < 7) {
				//ensure ghosts aren't too close to ninjaMan when game starts
				ghostLeft = 0;
				ghostTop = 0;
			}
		}
		ghostEntry = {
			ghostID: 'ghost' + String(x + 1),
			left: ghostLeft,
			top: ghostTop,
			leftPx: ghostLeftPx,
			topPx: ghostTopPx,
...
function drawScore(scoreFx, totalFx, ninjaFx) {
	document.getElementById('scoreBoard').innerHTML =
		'Lives: ' + ninjaFx.lives + '&nbsp;&nbsp;&nbsp;Score: ' + scoreFx + '/' + totalFx;
	document.getElementById('scoreBoard').innerHTML +=
		"<div style ='background-color: black; position: absolute; left: 360px; top: 4px; width: " +
		String(totalFx + 8) +
		"px; height: 32px'></div>";
	document.getElementById('scoreBoard').innerHTML +=
		"<div style ='background-color: magenta; position: absolute; left: 364px; top: 8px; width: " +
		scoreFx +
		"px; height: 24px'></div>";
}
...
function drawGhosts(badGhosts) {
	...
	document.getElementById('ghosts').innerHTML = contentFx;
}

function moveGhosts(ghostFx, ninjaFx) {
	let ghostRandomMovement = 0;
	let moveOptions = [];
	let ghostTempMove = ghostFx;
	let ghostMoveEnum = 0;
	ghostDelay++;
	if (ghostDelay === 4) {
		//delay implemented so ghosts can only move every 0.4 seconds
		ghostDelay = 0;
		for (let x = 0; x < ghostFx.length; x++) {
			ghostRandomMovement = Math.floor(Math.random() * 2);
			moveOptions = [];
			if (ghostFx[x].left === ninjaFx.left) {
				// move vertically if ghost has line of sight to ninjaman
				if (ghostFx[x].top < ninjaFx.top) {
					//down
					ghostTempMove = movementMapping(ghostFx[x].top, ghostFx[x].left, directionToEnum.down);
					if (world[ghostTempMove.top][ghostTempMove.left] > 0) {
						ghostFx[x].top = ghostTempMove.top;
					}
				} else if (ghostFx[x].top > ninjaFx.top) {
					//up
					ghostTempMove = movementMapping(ghostFx[x].top, ghostFx[x].left, directionToEnum.up);
					if (world[ghostTempMove.top][ghostTempMove.left] > 0) {
						ghostFx[x].top = ghostTempMove.top;
					}
				}
			} else if (ghostFx[x].top === ninjaFx.top) {
				// move horizontally if ghost has line of sight to ninjaman
				if (ghostFx[x].left < ninjaFx.left) {
					//right
					ghostTempMove = movementMapping(ghostFx[x].top, ghostFx[x].left, directionToEnum.right);
					if (world[ghostTempMove.top][ghostTempMove.left] > 0) {
						ghostFx[x].left = ghostTempMove.left;
					}
				} else if (ghostFx[x].left > ninjaFx.left) {
					//left
					ghostTempMove = movementMapping(ghostFx[x].top, ghostFx[x].left, directionToEnum.left);
					if (world[ghostTempMove.top][ghostTempMove.left] > 0) {
						ghostFx[x].left = ghostTempMove.left;
					}
				}
				...
				for (let y = 1; y < 5; y++) {
					ghostTempMove = movementMapping(ghostFx[x].top, ghostFx[x].left, y);
					if (world[ghostTempMove.top][ghostTempMove.left] > 0) {
						moveOptions.push(y);
					}
				}
				if (moveOptions.length < 1) {
					ghostMoveEnum = directionToEnum.stay;
				} else {
					ghostMoveEnum = moveOptions[Math.floor(Math.random() * moveOptions.length)];
				}
				ghostTempMove = movementMapping(ghostFx[x].top, ghostFx[x].left, ghostMoveEnum);
				ghostFx[x].left = ghostTempMove.left;
				ghostFx[x].top = ghostTempMove.top;
			}
		}
...
function movementMapping(topFx, leftFx, enumerationFx) {
	// function to convert movement direction enumeration into world coordinates
	let leftOutput = leftFx,
		topOutput = topFx;
...
function isSquareAWall(topFx, leftFx, enumerationFx) {
	//returns true if coordinates contain a wall space OR if the space being moved to contains a wall
	if (enumerationFx === undefined) {
		//enumeration is optional. if no enumerationFx not given assume no movement happens
		enumerationFx = 0;
	}
...
document.onkeydown = function (e) {
	//lets player use either arrow keys or WASD for moving ninjaman
...
function gameLoop() {
	//heartbeat of the game. executes all the game play functions
	if (gameTimer > 10) {
	...
	gameTimer++;
	if (gameTimer > gameTimeLimit || gameEnd > 0) {
		// end game if any conditions are met.
		if (gameEnd === 1) {
			document.getElementById('bannerHolder').innerHTML +=
				"<div id='banner' style='border-color: red; background-color: #fda5b4'><b>GAME OVER</b>" +
				'<br>You were killed by ghosts.<br><i>Refresh page to play again</i></div>';
		} else if (gameEnd === 2) {
			document.getElementById('bannerHolder').innerHTML +=
				"<div id='banner' style='border-color: green; background-color: #cceecc'><b>YOU WIN!</b>" +
				'<br>' +
				sushiScore +
				' sushi collected!<br><i>Refresh page to play again</i></div>';
		}
		return;
	}
